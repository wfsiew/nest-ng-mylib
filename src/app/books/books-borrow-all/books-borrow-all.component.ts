...
@Component({
  selector: 'app-books-borrow-all',
  templateUrl: './books-borrow-all.component.html',
  styleUrls: ['./books-borrow-all.component.scss']
})
export class BooksBorrowAllComponent implements OnInit {
  ...
  load() {
    this.isLoading = true;
    this.bookService.listBookBorrowAll(this.page, AppConstant.PAGE_SIZE).subscribe({
      next: (res: any) => {
        this.list = res.body;
